...
if (isset($_GET['searchTwo'])) {
	...
	$sql="SELECT * FROM `sas`.`schedule` WHERE `depDate` = '$depart_date' AND `retDate` = '$return_date' AND `fromLoc` = '$DepLoc' AND `toLoc` = '$RechLoc' ORDER BY `id`";
                    ...
                    if(mysqli_num_rows($result)){
                    	...
                    	echo "<center><h1>Sorry To Inform You That<br> We Do Not Have Buses That <br>Meet Your Requirements</h1></center>";
                    	die();
                    }
...
<?php if($avail == 1){
?>
<center><h4>Your Round Trip Bus's Total </h4> <h1><?php echo $totalSeat?></h1> <h4> Seat Is Available</h4></center>
<h2>You Requested Bus Of Following Information</h2>
...
<?php if($avail == 2){
?>
<center><h4>Your One Way Bus's Total </h4> <h1><?php echo $totalSeat?></h1> <h4> Seat Is Available</h4></center>
<h2>You Requested Bus Of Following Information</h2>
<form action="code/bookFlightAdminCode.php" method="POST">
	<center><table>
		<tr>
